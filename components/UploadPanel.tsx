...
export interface UploadPanelHandle {
  /** Load a file (e.g. a bundled sample) through the same validation + preview path. */
...
export const UploadPanel = forwardRef<UploadPanelHandle, UploadPanelProps>(
  function UploadPanel({ status, error, onSubmit, onReset }, ref) {
    ...
    // Let the parent feed in a sample without UploadPanel owning where it came from.
    useImperativeHandle(ref, () => ({ loadFile: (file: File) => handleFile(file) }), [
      handleFile,
    ]);
